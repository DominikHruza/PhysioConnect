...
export class AuthService {
  ...
  registerPatient(registration: RegistrationRequest){
    this.store.dispatch(loadingStart());
    return this.http.post('/api/patient/register', registration).pipe(
      catchError((error) => {
        this.store.dispatch(loadingEnd());
        return this.handleAuthError(error);
      }),
  ...
  private handleLoginSuccess(loginResponse: LoginResponse) {
    ...
    if (!roles.includes(UserRole.PRACTITIONER)) {
      this.handleAuthError(new HttpErrorResponse({ status: 403 }));
    }
